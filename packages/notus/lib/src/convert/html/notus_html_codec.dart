...
class _NotusHtmlEncoder extends Converter<Delta, String> {
  ...
  String convert(Delta input) {
    ...
    void _handleSpan(String text, Map<String, dynamic>? attributes, {bool hr = false, String? source}) {
      final style = NotusStyle.fromJson(attributes);
      currentInlineStyle = _writeInline(lineBuffer, text, style, currentInlineStyle, hr: hr, source: source);
    }
  ...
  void _writeTag(StringBuffer buffer, {bool close = false, required String tag}) {
...
class _NotusHtmlDecoder extends Converter<String, Delta> {
  ...
  Delta _parseElement(Element element, Delta delta, String type,
      {Map<String, dynamic>? attributes, String? listType, next, isNewLine, inBlock}) {
    if (type == 'block') {
      ...
      element.nodes.asMap().forEach((index, node) {
        ...
        delta = _parseNode(node, delta, next,
            isNewLine: element.localName == 'li' || element.localName == 'p' || element.localName == 'div',
      ...
      if (element.localName == 'img') {
        delta = delta..insert(BlockEmbed.image(element.attributes['src']!).toJson());
      }
      ...
      if (element.children.isEmpty) {
        if (attributes['a'] != null) {
          ...
          if ((isNewLine == null || (isNewLine != null && !isNewLine)) && inBlock == null) delta.insert('\n');
        } else {
          if (next != null && next.runtimeType == Element && next.localName == 'br') {
        ...
        for (var node in element.nodes) {
          if (node.runtimeType == Element) {
            ...
            delta = _parseElement(node, delta, elementType, attributes: attributes, next: next);
